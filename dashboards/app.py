...
class WeatherEnergyDashboard:
    ...
    def _initialize_components(self):
         try:
             self.pipeline = DataPipeline("config/config.yaml")
    ...
    def render_sidebar(self) -> Dict:
        ...
        # Safe handling of config structure
        if isinstance(self.config, dict) and 'cities' in self.config:
            available_cities = [city["name"] for city in self.config["cities"]]
        elif isinstance(self.config, list) and len(self.config) > 0:
            # If config is a list, try to get cities from first dict
            first_config = self.config[0] if isinstance(self.config[0], dict) else {}
            available_cities = list(first_config.get('cities', {}).keys())
        else:
            available_cities = []
    ...
    def render_geographic_overview(self, data: Dict[str, pd.DataFrame]):
        ...
        for city, df in data.items():
            if len(df) > 1:
                ...
                if isinstance(self.config, dict) and 'cities' in self.config:
                    city_info = next((c for c in self.config['cities'] if c['name'] == city), {})
                else:
                    city_info = {}
